...
http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && path === '/api/images') {
    ...
    if (!UNSPLASH_KEY) { sendJSON(res, 200, {photos:[]}); return; }
    try {
      const encoded = encodeURIComponent(query.slice(0, 50));
      const d = await httpsGet(
        'api.unsplash.com',
        `/search/photos?query=${encoded}&per_page=4&orientation=squarish`
      );
      // httpsGetではヘッダーを渡せないので専用実装
      throw new Error('USE_FULL');
    } catch(e) {
      if (e.message !== 'USE_FULL') {
        sendJSON(res, 200, {photos:[]});
        return;
      }
    }
    // Authorization ヘッダーが必要なのでhttpsPostのGET版を使う
    try {
      const encoded = encodeURIComponent(query.slice(0, 50));
      const d = await new Promise((resolve, reject) => {
        const req2 = https.request(
          { hostname:'api.unsplash.com',
            path:`/search/photos?query=${encoded}&per_page=4&orientation=squarish`,
            method:'GET',
          ...
          r => {
            ...
            r.on('data', c => { raw += c; });
            r.on('end', () => {
              if (r.statusCode >= 400) { reject(new Error('HTTP ' + r.statusCode)); return; }
              try { resolve(JSON.parse(raw)); } catch(e2) { reject(e2); }
            });
          }
        );
      ...
      console.warn('[unsplash]', e2.message);
      sendJSON(res, 200, {photos:[]});
    }
    return;
  }
      ...
      try {
        const d = await new Promise((resolve, reject) => {
          const r2 = https.request(
            { hostname: FIREBASE_DB_URL.replace('https://',''),
